...
if (!digestPaths.length) {
  emitErr('Usage: node tools/foreground.mjs <digest.json> [more...] [--out <dir>] [--bars N-M] [--json]');
  process.exit(2);
}
...
const loaded = digestPaths.map((p) => {
  try {
    ...
    emitErr(`Cannot read digest "${p}": ${e.message}`);
    process.exit(2);
  }
});
...
if (!Array.isArray(primary.bars)) {
  emitErr(`Digest "${digestPaths[0]}" has no bars[] — re-run piano-extract.`);
  process.exit(1);
}
if (!primary.bars.some((b) => (b.foregroundEvidence || []).length)) {
  emitErr(`Digest "${digestPaths[0]}" carries no foregroundEvidence — it predates `
...
if (json) emit(JSON.stringify(doc, null, 2));
else {
  emit(`${primary.song}: ${doc.summary.bars} bar(s) analyzed -> ${jsonPath}, ${mapPath}`);
  emit(`   cross-sources: ${others.length}  |  ambiguous bars: `
  ...
  if (doc.summary.ambiguousBars.length) {
    emit('   review the ambiguous bars in foreground-map.md before locking a melody contract.');
  }
}
process.exit(0);
